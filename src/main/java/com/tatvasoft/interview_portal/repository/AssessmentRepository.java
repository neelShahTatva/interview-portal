...
public interface AssessmentRepository extends JpaRepository<Assessment, Long> {
    ...
    List<Assessment> findAllByCandidateId(Long candidateId);

    boolean existsByCandidateIdAndIsActiveTrue(Long candidateId);

    List<Assessment> findByIsActiveTrue();

    Optional<Assessment> findByCandidateIdAndIsActiveTrue(Long candidateId);

    List<Assessment> findByCompletedAtBeforeAndIsActiveTrue(LocalDateTime cutoff);
}